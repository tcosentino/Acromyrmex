...
var OurEditor = function (_React$Component) {
  ...
  function OurEditor(props) {
    ...
    if (props && props.input && props.input.value && props.input.value.length) {
      _this.setState(function (prevState) {
        return {
          editorState: _draftJs.EditorState.push(prevState.editorState, _this.getStateFromMarkdown(props.input.value, props.options))
        };
      });
    }
  ...
  _createClass(OurEditor, [{
    ...
    value: function onSuggestionSearch(functionInfo, _ref) {
      ...
      var innerSuggestions = this.state.suggestions;

      var newSuggestions = [].concat(_toConsumableArray(innerSuggestions));

      // set our normal suggestions
      newSuggestions[functionInfo.index] = (0, _suggestionsFilter2.default)(value, newSuggestions[functionInfo.index]);
    ...
    value: function addMentionPlugin(functionInfo) {
      ...
      var nSuggestions = [].concat(_toConsumableArray(suggestions));
